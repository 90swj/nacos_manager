...
class ConnectionProvider with ChangeNotifier {  
  ...
  /// 修改：返回 Map 以便获取 content, desc, config_tags  
  Future<Map<String, dynamic>> getConfigContent({required String dataId, required String group, required String tenant}) async {  
    try {  
      final response = await _activeClient!.get(  
        '/nacos/v1/cs/configs',  
      ...
      // 兼容处理：如果返回的是字符串（极少数情况），包装成 map  
      if (response.data is String) {  
        return {'content': response.data};  
      }  
        
      // 正常返回 JSON 对象，包含 content, config_tags, desc 等  
  ...
  Future<bool> publishConfig({  
    ...
    try {  
      final formData = FormData.fromMap({  
        ...
        if (tags != null && tags.isNotEmpty) 'config_tags': tags, // 传递给 Nacos  
      });  
